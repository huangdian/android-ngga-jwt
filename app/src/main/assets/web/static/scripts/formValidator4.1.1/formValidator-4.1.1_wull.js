...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		//modify by wull 第一个也获得焦点
		if(settings.mode == "SingleTip"){settings.errorFocus=true};
		//如果填写了表单和按钮，就注册验证事件
		if(settings.formID!=""){
				...
				else
				{
					...
					if(!returnObj.isValid)
					{
						...
						if(auto)
						{
							...
							//modify by wull
							if(initConfig.mode != "AlertTip"&&initConfig.mode != "SingleTip"){$.formValidator.setTipState(this,"onShow",$.formValidator.getStatusText(this,setting.onCorrect))};
						}
						else
						{
