...
define(function(require, exports, module) { //参数名字不能改
  ...
  exports.init =function() {   
     ...
     var op = {
        ...
        url: "/calendar/leader/index" ,  
        quickAddUrl: "/calendar/leader/quickadd" ,  
        quickUpdateUrl: "/calendar/leader/quickupdate" ,  
        quickDeleteUrl:  "/calendar/leader/quickdelete",//快速删除日程的
        leaderList:queryLeaderList//查询领导列表
...
//    op.height = _MH-90;
    op.height = _MH-120;
    op.eventItems =[];
    var calendar=$("#xgcalendarp").bcalendar(op);
    var p =calendar.BcalGetOp();
    if (p && p.datestrshow) {       
    ...
    function queryLeaderList(){
    	var select=$('<select id="bbit-cal-leader" class="textbox-fill-input"></select>');
    	var list=$(".leaders");
    	for(var i=0;i<list.length;i++){
    		var leader=$(list.get(i));
	    	var opt=$("<option></option>");
	    	$(opt).text($(leader).text());
	    	var uid=$(leader).attr('uid');
	    	$(opt).attr("value",uid);
	    	if($(leader).hasClass("now")){
	    		opt.attr("selected",true);
	    	}
	    	select.append(opt);
    	}
    	var str=select.prop("outerHTML");
    	return str;
    }
